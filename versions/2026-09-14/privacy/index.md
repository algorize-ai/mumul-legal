...
description: "무물 수능 개인정보처리방침 2026-09-14 개정안 (시행일 지정: 2026-09-16)"
...
자체 서버의 광고 정보 비수집과 Google의 공개 보유 기준을 반영한 초안입니다. 아래 제8조의 실제 수령자·처리 국가·수령자별 보유 기준 및 국외이전 근거·동의·철회 절차는 최종 확인이 필요합니다.
...
| 항목 | 보관 기간 및 파기 기준 |
| --- | --- |
| 계정과 연결된 제출 문제 이미지, 추출 텍스트, 문제·질문, 대화, AI 답변·해설 및 관련 메타데이터 | 문제풀이와 대화 제공을 위해 계정 유지 기간 동안 보관. 보유기간이 경과하거나 처리 목적이 달성되거나 이용자의 적법한 삭제 요청이 처리되면 삭제. 앱 내 데이터 삭제의 대상·처리 순서는 제9조에 따름 |
...
Google이 직접 처리하는 광고 관련 정보에는 [Google 개인정보처리방침](https://policies.google.com/privacy?hl=ko)과 [데이터 보유 기준](https://policies.google.com/technologies/retention?hl=ko)이 적용됩니다. 보유기간은 정보의 종류, 처리 목적, 이용자의 설정 및 삭제 요청, 보안·부정 이용 방지와 법적 보존 필요에 따라 달라집니다. 서비스는 해당 정보를 자체 서버에 보관하지 않으며 Google 시스템의 개별 삭제 시점을 직접 결정하지 않습니다. Google이 공개한 광고 서버 로그의 일부 익명화 시점은 모든 광고 관련 정보의 삭제 완료 시점을 의미하지 않습니다. 추가 광고 파트너의 처리 조건과 국외이전 사항은 제8조의 확정 대상입니다.
...
서비스는 아래 SDK를 통해 광고 및 광고 개인정보 설정 관련 정보를 자동으로 처리할 수 있습니다. 이 처리는 이용자가 문제 이미지와 질문을 제출하는 AI 처리와 구분됩니다. 광고 플랫폼은 Google AdMob만 사용하며, 다른 광고 네트워크를 연결하는 추가 미디에이션과 Firebase Analytics는 사용하지 않습니다.
...
Google Mobile Ads SDK와 UMP SDK를 통해 Google 및 AdMob 계정에 활성화된 광고 파트너가 광고 관련 정보를 직접 수신하고 대한민국 밖에서 처리할 수 있습니다. 해외에서 정보가 처리된다는 사실만으로 서비스에 별도 국외이전 동의 의무가 있다고 단정하지 않습니다. [개인정보보호위원회 안내](https://pipc.go.kr/np/default/page.do?mCode=D060040000)에 따라 해외 사업자의 직접 수집과 서비스가 개인정보를 제공·처리위탁·보관하는 국외이전을 구분하여 실제 처리 관계를 확인해야 합니다. 이 광고 처리는 핵심 AI 문제풀이와 목적·필요성이 다르므로, 제6조에 적은 계약 체결·이행 근거를 광고 처리에 자동으로 적용하지 않습니다.

Google의 공개 개인정보처리방침은 Google LLC 및 계열사의 광고 서비스에 적용되며, Google은 여러 국가에서 정보를 처리할 수 있다고 설명합니다. 다만 무물의 실제 광고 수령자·연락처와 처리 국가는 추가 광고 파트너를 포함한 AdMob 계정 설정 및 적용되는 처리 조건과 대조해 확정해야 합니다. 광고대금 지급 계약의 상대방 법인만으로 개인정보의 실제 수령자나 처리 국가를 단정하지 않습니다.

이 초안에는 자체 서버의 광고 정보 비수집과 Google의 공개 데이터 보유 기준을 반영했습니다. 남은 확인 사항은 다음과 같습니다.

- Google AdMob을 통해 실제로 관여하는 광고 파트너와 각 수령자의 연락처·처리 국가, Google 측 입찰 및 publisher first-party ID 설정. 별도 미디에이션과 Firebase Analytics를 사용하지 않는 운영 구성이 계정에도 적용되어 있는지 확인
- 실제 수령자별 이전 항목, 시기·방법, 목적 및 적용되는 보유·삭제 기준
- Google 등의 직접 수집과 서비스의 국외이전 중 실제 처리 관계에 해당하는 유형, 그에 따른 법적 근거와 필요한 동의·거부·철회 절차 및 그 효과

UMP의 광고 요청 가능 상태만으로 한국 이용자의 광고 국외이전에 필요한 법적 요건이 충족되었다고 간주하지 않습니다. 별도 동의를 근거로 정하는 경우 해당 고지와 동의·철회 절차를 실제로 마련해야 합니다. 광고가 포함된 문서를 최종 확정하고 운영 빌드를 출시하기 전에 위 항목을 반영해야 하며, 시행일을 지정한 사실이 이 확인이나 구현의 완료를 의미하지 않습니다.
